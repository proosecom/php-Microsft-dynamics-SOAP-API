...
class CrmAuth {
	var $CrmAuthenticationHeaderobj;
	
	
	public function __construct($CrmAuthenticationHeader = ""){
		$this->CrmAuthenticationHeaderobj = $CrmAuthenticationHeader ;
		//$CI =& get_instance();
		//$CI->load->library('../msdynamics/CrmAuthenticationHeader');		
	}
	/**
	...
	public function GetHeaderOnline($username, $password, $url) {		
		...
		$this->CrmAuthenticationHeaderobj->Expires = $tokenExpires;		
		$this->CrmAuthenticationHeaderobj->Header = $this->CreateSoapHeaderOnline ( $url, $keyIdentifer, $token1, $token2 );			
		return $this->CrmAuthenticationHeaderobj;
	}
	...
	function GetUrnOnline($url) {
		...
		if (strpos ( strtoupper ( $url ), "CRM8.DYNAMICS.COM" )) {
			return "crmind:dynamics.com";
		}
		if (strpos ( strtoupper ( $url ), "CRM3.DYNAMICS.COM" )) {
			return "crmcan:dynamics.com";
		}
	...
	function GetHeaderOnPremise($username, $password, $url) {
		...
		$CI->crmauthenticationheader->Expires = $tokenExpires;
		$CI->crmauthenticationheader->Header = $this->CreateSoapHeaderOnPremise ( $url, $keyIdentifer, $token1, $token2, $x509IssuerName, $x509SerialNumber, $signatureValue, $digestValue, $created, $expires );
		
		return $CI->crmauthenticationheader;
	}
